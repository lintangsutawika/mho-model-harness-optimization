...
class ScriptArgs(U.ExecuteTrainConfig):
    ...
    # HF-format checkpoint path (Megatron backend). A format string: "{}" is filled with
    # the rollout id, e.g. ".../hf/step_{}" -> .../hf/step_100. Empty = no HF save.
    save_hf: str = os.environ.get("MILES_SAVE_HF", "")
    save_traces_dir: str = ""
...
def execute(args: ScriptArgs):
    ckpt_args = (
        ...
        + (f"--save-hf {args.save_hf} " if args.save_hf else "")
    )
    rollout_args = (
        f"--prompt-data {args.prompt_data} "
        "--input-key prompt "
        "--metadata-key metadata "
        # Qwen3.5 is a VL checkpoint, so miles' data_source loads an AutoProcessor
        # (non-None). With a processor present, Dataset requires each prompt to be a
        # chat-message list; --apply-chat-template converts our string prompt into
        # [{"role":"user","content":...}] (no media) and stores the templated prompt.
        # Only affects the dataset seed; the harbor agent builds its own prompts.
        "--apply-chat-template "
        "--rollout-shuffle "
    ...
    agent_args = agentic_train_args(tito_model="qwen35", session_server_workers=32)
    # Redirect the agent function to our shim so the rollout worker imports
    # mho.backends.local_singularity.remote_env (arming the EnvironmentType.SINGULARITY
    # -> RemoteSingularityEnvironment registry swap) BEFORE any trial builds its env.
    # Without this the worker uses harbor's stock SingularityEnvironment and tries to run
    # apptainer nested inside the Miles container (fails: no apptainer binary there).
    agent_args = agent_args.replace(
        "--custom-agent-function-path harbor_agent_function.run",
        "--custom-agent-function-path mho.backends.local_singularity.miles_agent_fn.run",
    )
    misc_args = (
